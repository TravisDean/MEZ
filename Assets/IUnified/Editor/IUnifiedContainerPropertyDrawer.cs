...
public class IUnifiedContainerPropertyDrawer : PropertyDrawer
{
    ...
    private static string BuildEditorResultString(string resultType, Object @object)
    {
        if(@object != null)
        {
            ...
            if(component != null)
            {
                return string.Format("{0} ( {1} )", component.gameObject.name, IUnifiedContainerBase.IUnifiedContainerBase.ConstructResolvedName(@object.GetType()));
            }
    ...
    private static Type GetContainerType(SerializedProperty property)
    {
        ...
        {
            var fieldInfo = type.GetField(field, FieldBindingFlags);
            if(fieldInfo == null)
            {
                return null;
            }
